...
public enum StrategyDefinition {
    ...
    RSI_14("RSI Mean Reversion(14)",s->{var i=new RSIIndicator(new ClosePriceIndicator(s),14);return thresholds(i,30,70);}),
    ...
    STOCHASTIC_14("Stochastic Mean Reversion(14)",s->{var k=new StochasticOscillatorKIndicator(s,14);return thresholds(k,20,80);}),
    CCI_20("CCI Mean Reversion(20)",s->{var i=new CCIIndicator(s,20);return thresholds(i,-100,100);}),
    ADX_14("ADX/DMI(14)",s->{var plus=new PlusDIIndicator(s,14);var minus=new MinusDIIndicator(s,14);var adx=new ADXIndicator(s,14);var strength=new ConstantIndicator<>(s,s.numFactory().numOf(25));return neutral(new AndRule(new OverIndicatorRule(adx,strength),new OverIndicatorRule(plus,minus)),new AndRule(new OverIndicatorRule(adx,strength),new OverIndicatorRule(minus,plus)));}),
    WILLIAMS_R_14("Williams %R Mean Reversion(14)",s->{var i=new WilliamsRIndicator(s,14);return thresholds(i,-80,-20);}),
    ...
    ROC_12("ROC Momentum(12)",s->zeroMomentum(new ROCIndicator(new ClosePriceIndicator(s),12))),
    CHAIKIN_MONEY_FLOW("Chaikin Money Flow Momentum(20)",s->zeroMomentum(new ChaikinMoneyFlowIndicator(s,20))),
