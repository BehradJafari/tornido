...
public class TornadoController {
 ...
 @GetMapping("/method-report") MethodReport method(@RequestParam String method,@RequestParam(defaultValue="3600")long horizon){ReportService.requireSupportedHorizon(horizon);var ps=predictions.findByMethodNameAndSignalVersionOrderByPredictedAtDesc(method,2).stream().filter(p->p.getHorizonSeconds()==horizon).toList();long graded=ps.stream().filter(p->p.getOutcome()==Outcome.CORRECT||p.getOutcome()==Outcome.INCORRECT).count(),targetCorrect=ps.stream().filter(p->p.getOutcome()==Outcome.CORRECT).count(),directionalCorrect=ps.stream().filter(this::directionCorrect).count();return new MethodReport(method,ps.size(),graded,targetCorrect,graded==0?0:targetCorrect*100.0/graded,directionalCorrect,graded==0?0:directionalCorrect*100.0/graded,ps);}
 ...
 @GetMapping("/predictions") List<Prediction> history(@RequestParam(required=false)String coin,@RequestParam(required=false)String method,@RequestParam(required=false)@DateTimeFormat(iso=DateTimeFormat.ISO.DATE_TIME)Instant from,@RequestParam(required=false)@DateTimeFormat(iso=DateTimeFormat.ISO.DATE_TIME)Instant to){return predictions.searchCurrent(2,blank(coin),blank(method),from==null?Instant.now().minus(Duration.ofDays(30)):from,to==null?Instant.now():to);}
 @GetMapping("/predictions/history") List<Prediction> allHistory(@RequestParam(required=false)String coin,@RequestParam(required=false)String method,@RequestParam(required=false)@DateTimeFormat(iso=DateTimeFormat.ISO.DATE_TIME)Instant from,@RequestParam(required=false)@DateTimeFormat(iso=DateTimeFormat.ISO.DATE_TIME)Instant to){return predictions.searchAll(blank(coin),blank(method),from==null?Instant.now().minus(Duration.ofDays(30)):from,to==null?Instant.now():to);}
 @GetMapping("/leaderboard") List<LeaderRow> board(@RequestParam(required=false)String coin,@RequestParam(defaultValue="7d")String window,@RequestParam(defaultValue="3600")long horizon){ReportService.requireSupportedHorizon(horizon);Instant from=Instant.now().minus(parseWindow(window));return predictions.leaderboard(blank(coin),from,horizon).stream().map(r->{long total=((Number)r[1]).longValue(),targetCorrect=((Number)r[2]).longValue(),directionalCorrect=((Number)r[3]).longValue();return new LeaderRow((String)r[0],total,targetCorrect,total==0?0:targetCorrect*100.0/total,directionalCorrect,total==0?0:directionalCorrect*100.0/total);}).sorted(Comparator.comparingDouble(LeaderRow::targetHitRate).reversed()).toList();}
 ...
 private boolean directionCorrect(Prediction p){if((p.getOutcome()!=Outcome.CORRECT&&p.getOutcome()!=Outcome.INCORRECT)||p.getPriceAtGrading()==null)return false;int c=p.getPriceAtGrading().compareTo(p.getPriceAtPrediction());return p.getPredictedDirection()==Direction.UP?c>0:c<0;}
 ...
 private RunSummary summary(AnalysisRun r){var ps=predictions.findByAnalysisRunIdOrderByCoinSymbolAscMethodNameAsc(r.getId());long pending=ps.stream().filter(p->p.getOutcome()==Outcome.PENDING).count(),ungradable=ps.stream().filter(p->p.getOutcome()==Outcome.UNGRADABLE).count(),correct=ps.stream().filter(p->p.getOutcome()==Outcome.CORRECT).count(),graded=ps.stream().filter(p->p.getOutcome()==Outcome.CORRECT||p.getOutcome()==Outcome.INCORRECT).count(),directionalCorrect=ps.stream().filter(this::directionCorrect).count();return new RunSummary(r.getId(),r.getName(),r.getCreatedAt(),r.getHorizonSeconds(),r.getStatus().name(),ps.size(),pending,ungradable,correct,graded==0?0:correct*100.0/graded,directionalCorrect,graded==0?0:directionalCorrect*100.0/graded,r.getErrorCount());}
 ...
 public record RunSummary(long id,String name,Instant createdAt,long horizonSeconds,String status,long predictions,long pending,long ungradable,long targetCorrect,double targetHitRate,long directionalCorrect,double directionalAccuracy,int errors){}
 public record RunReport(RunSummary summary,List<Prediction> predictions){}
 public record MethodReport(String method,long total,long graded,long targetCorrect,double targetHitRate,long directionalCorrect,double directionalAccuracy,List<Prediction> predictions){}
 public record MethodDto(String id,String name){} public record LeaderRow(String method,long total,long targetCorrect,double targetHitRate,long directionalCorrect,double directionalAccuracy){}
}
