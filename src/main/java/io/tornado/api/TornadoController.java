...
public class TornadoController {
 ...
 @GetMapping("/method-report") MethodReport method(@RequestParam String method,@RequestParam(defaultValue="0")long horizon){var ps=predictions.findByMethodNameOrderByPredictedAtDesc(method).stream().filter(p->horizon==0||p.getHorizonSeconds()==horizon).toList();long graded=ps.stream().filter(p->p.getOutcome()!=Outcome.PENDING).count(),correct=ps.stream().filter(p->p.getOutcome()==Outcome.CORRECT).count();return new MethodReport(method,ps.size(),graded,correct,graded==0?0:correct*100.0/graded,ps);}
 ...
 @GetMapping("/reports/coins") List<ReportService.CoinReport> coinReports(@RequestParam(defaultValue="3")@Min(1)int minSamples,@RequestParam(defaultValue="0")long horizon){return reports.coinReports(minSamples,horizon);}
 @GetMapping("/reports/mixes") List<ReportService.MixAccuracy> mixReports(@RequestParam(defaultValue="3")@Min(1)int minSamples,@RequestParam(defaultValue="3")int size,@RequestParam(defaultValue="0")long horizon){return reports.mixReports(minSamples,size,horizon);}
 @GetMapping("/reports/coin-mixes") List<ReportService.CoinMixReport> coinMixReports(@RequestParam(defaultValue="3")@Min(1)int minSamples,@RequestParam(defaultValue="3")int size,@RequestParam(defaultValue="0")long horizon){return reports.coinMixReports(minSamples,size,horizon);}
 @GetMapping("/reports/super") ReportService.SuperReport superReport(@RequestParam(defaultValue="5")@Min(1)int minSamples,@RequestParam(defaultValue="0")long horizon){return reports.superReport(minSamples,horizon);}
