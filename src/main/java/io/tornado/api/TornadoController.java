...
public class TornadoController {
 ...
 @GetMapping("/method-report") MethodReport method(@RequestParam String method,@RequestParam(defaultValue="3600")long horizon){var ps=predictions.findByMethodNameAndSignalVersionOrderByPredictedAtDesc(method,2).stream().filter(p->p.getHorizonSeconds()==horizon).toList();long graded=ps.stream().filter(p->p.getOutcome()!=Outcome.PENDING).count(),targetCorrect=ps.stream().filter(p->p.getOutcome()==Outcome.CORRECT).count(),directionalCorrect=ps.stream().filter(this::directionCorrect).count();return new MethodReport(method,ps.size(),graded,targetCorrect,graded==0?0:targetCorrect*100.0/graded,directionalCorrect,graded==0?0:directionalCorrect*100.0/graded,ps);}
 ...
 private boolean directionCorrect(Prediction p){if(p.getOutcome()==Outcome.PENDING||p.getPriceAtGrading()==null)return false;int c=p.getPriceAtGrading().compareTo(p.getPriceAtPrediction());return p.getPredictedDirection()==Direction.UP?c>0:c<0;}
