    ...
    public byte[] superAnalysis(){
        ...
        try(var workbook=new XSSFWorkbook();var output=new ByteArrayOutputStream()){
            ...
            for(int h=0;h<HORIZONS.length;h++){
                Sheet sheet=workbook.createSheet(SHEETS[h]);List<String> headings=new ArrayList<>(List.of("Coin","Time slice"));for(int size=1;size<=6;size++)headings.addAll(List.of("Best "+size+" methods","Accuracy","All predictions","Decisive predictions","Correct predictions","Same direction","Same-direction correct"));writeRow(sheet.createRow(0),header,headings.toArray());
                int rowIndex=1;for(var coin:coinList){Row row=sheet.createRow(rowIndex++);row.createCell(0).setCellValue(coin.getSymbol());row.createCell(1).setCellValue(SHEETS[h]);var data=byKey.get(coin.getSymbol()+":"+HORIZONS[h]);Map<Integer,ReportService.MixAccuracy> mixes=new HashMap<>();if(data!=null)data.bestMixes().forEach(x->{if(x.mix()!=null)mixes.put(x.size(),x.mix());});int column=2;for(int size=1;size<=6;size++){var mix=mixes.get(size);if(mix==null){column+=7;continue;}row.createCell(column++).setCellValue(String.join(" + ",mix.methods()));Cell accuracy=row.createCell(column++);accuracy.setCellValue(mix.accuracy()/100);accuracy.setCellStyle(percent);row.createCell(column++).setCellValue(mix.totalPredictions());row.createCell(column++).setCellValue(mix.samples());row.createCell(column++).setCellValue(mix.correct());row.createCell(column++).setCellValue(mix.sameDirectionPredictions());row.createCell(column++).setCellValue(mix.sameDirectionCorrect());}}
                sheet.createFreezePane(2,1);sheet.setAutoFilter(new org.apache.poi.ss.util.CellRangeAddress(0,Math.max(0,rowIndex-1),0,43));autosize(sheet,44);
            }
