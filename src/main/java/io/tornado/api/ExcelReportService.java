    ...
    public byte[] superAnalysis(int signalVersion){return superAnalysis(signalVersion,1);}public byte[] superAnalysis(int signalVersion,int tpLevel){
        ...
        try(var workbook=new XSSFWorkbook();var output=new ByteArrayOutputStream()){
            ...
            for(int h=0;h<HORIZONS.length;h++){
                Sheet sheet=workbook.createSheet(SHEETS[h]);List<String> headings=new ArrayList<>(List.of("Coin","Time slice"));for(int size=1;size<=6;size++)headings.addAll(List.of("Best "+size+" methods","TP"+tpLevel+" hit rate","Directional accuracy","All predictions","Decisive predictions","TP"+tpLevel+" hits","Directional correct","Same direction","Same-direction TP"+tpLevel+" hits"));writeRow(sheet.createRow(0),header,headings.toArray());
