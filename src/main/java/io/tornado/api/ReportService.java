...
public class ReportService {
    ...
    public MoneyReport moneyReport(MoneyRequest request){
        ...
        int executed=trades.size(),peakConcurrent=peakConcurrency(candidates);double totalMargin=request.tradeAmount()*executed,peakMargin=request.tradeAmount()*peakConcurrent,profitWinRate=executed==0?0:profitable*100.0/executed,targetHitRate=executed==0?0:targetHits*100.0/executed,roi=peakMargin==0?0:netTotal*100/peakMargin,average=executed==0?0:netTotal/executed;return new MoneyReport(request.coin().toUpperCase(),request.horizon(),List.copyOf(selected),"SIMPLE_APPROXIMATE",executed,request.tradeAmount(),request.leverage(),totalMargin,peakConcurrent,peakMargin,grossTotal,totalCosts,netTotal,peakMargin+netTotal,roi,profitable,losing,breakEven,profitWinRate,targetHits,executed-targetHits,targetHitRate,liquidations,realizedDrawdown,average,trades);
    }
    ...
    public record MoneyReport(String coin,long horizon,List<String> methods,String liquidationModel,int executedTrades,double tradeAmount,int leverage,double totalMarginAllocated,int peakConcurrentTrades,double peakMarginRequired,double grossPnl,double totalCosts,double netPnl,double endingValue,double roiOnPeakMarginPercent,int profitableTrades,int losingTrades,int breakEvenTrades,double profitWinRate,int targetHits,int targetMisses,double targetHitRate,int liquidations,double realizedPnlDrawdown,double averageNetPnlPerTrade,List<MoneyTrade> trades){}
}
