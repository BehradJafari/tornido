...
public class ReportService {
    ...
    private List<MixAccuracy> calculateMixes(List<ReportRow> source,int minSamples,int size){return calculateMixesBySizes(source,minSamples,size).getOrDefault(size,List.of());}
    private Map<Integer,List<MixAccuracy>> calculateMixesBySizes(List<ReportRow> source,int minSamples,int... sizes){
        ...
        for(GroupSignals group:groups.values())for(int size:sizes){if(size<1||Long.bitCount(group.methods)<size)continue;Map<Long,MixScore> scores=scoresBySize.get(size);combinations(group.methods,size,0L,mix->{int ups=Long.bitCount(mix&group.ups);MixScore score=scores.computeIfAbsent(mix,x->new MixScore());score.totalPredictions++;boolean sameDirection=ups==0||ups==size,predictedUp=ups*2>size,targetHit=group.targetHit(predictedUp),directionHit=group.directionHit(predictedUp);if(sameDirection){score.sameDirectionPredictions++;if(targetHit)score.sameDirectionCorrect++;}if(ups*2==size)return;score.samples++;if(targetHit)score.targetCorrect++;if(directionHit)score.directionalCorrect++;});}
    ...
    private static class GroupSignals {long methods,ups;final java.math.BigDecimal entry,exit;GroupSignals(ReportRow p){entry=p.getPriceAtPrediction();exit=p.getPriceAtGrading();}void put(int index,Direction direction){long bit=1L<<index;methods|=bit;if(direction==Direction.UP)ups|=bit;else ups&=~bit;}boolean targetHit(boolean predictedUp){if(exit==null)return false;var change=exit.subtract(entry).divide(entry,12,java.math.RoundingMode.HALF_UP);return predictedUp?change.compareTo(MINIMUM_CORRECT_MOVE)>=0:change.compareTo(MINIMUM_CORRECT_MOVE.negate())<=0;}boolean directionHit(boolean predictedUp){if(exit==null)return false;int comparison=exit.compareTo(entry);return predictedUp?comparison>0:comparison<0;}}
