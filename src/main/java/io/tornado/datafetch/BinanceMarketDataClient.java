...
public class BinanceMarketDataClient {
    ...
    public BarSeries candles(String pair){
        ...
        for(JsonNode r:rows) series.barBuilder().timePeriod(period).endTime(Instant.ofEpochMilli(r.get(6).asLong())).openPrice(r.get(1).asText()).highPrice(r.get(2).asText()).lowPrice(r.get(3).asText()).closePrice(r.get(4).asText()).volume(r.get(5).asText()).add();
        return series;
    }
