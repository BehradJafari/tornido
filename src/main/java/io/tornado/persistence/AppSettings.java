...
public class AppSettings {
    ...
    @Column(name="minimum_notification_win_rate_percent",nullable=false,precision=8,scale=4) private java.math.BigDecimal minimumNotificationWinRatePercent=new java.math.BigDecimal("60.00");
    ...
    public int getMinimumMixSimulationTrades(){return minimumMixSimulationTrades;} public java.math.BigDecimal getMinimumNotificationWinRatePercent(){return minimumNotificationWinRatePercent;} public java.math.BigDecimal getMixTradeStopLossPercent(){return mixTradeStopLossPercent;} public boolean isTelegramDailyReportEnabled(){return telegramDailyReportEnabled;}
    ...
    public void updateMixSignals(int minimumTrades,TpSlLevels levels,boolean dailyReportEnabled){updateMixSignals(minimumTrades,levels,minimumNotificationWinRatePercent,dailyReportEnabled);}
    public void updateMixSignals(int minimumTrades,TpSlLevels levels,java.math.BigDecimal minimumNotificationWinRatePercent,boolean dailyReportEnabled){if(minimumTrades<1||minimumTrades>100000)throw new IllegalArgumentException("minimum mix simulation trades must be between 1 and 100,000");if(levels==null)throw new IllegalArgumentException("TP/SL levels are required");if(minimumNotificationWinRatePercent==null||minimumNotificationWinRatePercent.compareTo(java.math.BigDecimal.ZERO)<0||minimumNotificationWinRatePercent.compareTo(new java.math.BigDecimal("100"))>0)throw new IllegalArgumentException("minimum notification win rate must be between 0 and 100 percent");minimumMixSimulationTrades=minimumTrades;this.minimumNotificationWinRatePercent=minimumNotificationWinRatePercent;takeProfit1Percent=levels.tp1();takeProfit2Percent=levels.tp2();takeProfit3Percent=levels.tp3();stopLoss1Percent=levels.sl1();stopLoss2Percent=levels.sl2();stopLoss3Percent=levels.sl3();mixTradeStopLossPercent=levels.sl2();telegramDailyReportEnabled=dailyReportEnabled;}
