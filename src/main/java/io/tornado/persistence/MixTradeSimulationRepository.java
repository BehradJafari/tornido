...
public interface MixTradeSimulationRepository extends JpaRepository<MixTradeSimulation,Long>{
 ...
 @Query("select case when count(s) > 0 then true else false end from MixTradeSimulation s where s.status = :status and s.coin.pair = :pair")boolean existsOpenForPair(@Param("status")MixTradeSimulation.Status status,@Param("pair")String pair);
}
