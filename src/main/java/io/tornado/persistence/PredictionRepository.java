...
public interface PredictionRepository extends JpaRepository<Prediction,Long> {
    List<Prediction> findByOutcomeOrderByPredictedAtAsc(Outcome outcome);
    ...
    @Query("select p.methodName, count(p), sum(case when p.outcome='CORRECT' then 1 else 0 end) from Prediction p where p.outcome<>'PENDING' and p.predictedAt>=:from and (:coin is null or p.coin.symbol=:coin) and (:horizon=0 or p.horizonSeconds=:horizon) group by p.methodName")
    ...
    @Query("select p from Prediction p where p.outcome <> 'PENDING' order by p.predictedAt desc") List<Prediction> findAllGraded();
}
