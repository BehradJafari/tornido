...
class BestMixServiceTest {
    @Test void refreshReplacesPreviousRanksAndStoresAtMostTopThreePerSize(){
        ...
        for(int selectedSize:List.of(2,3,4,5,6,7,8)){List<BestMethodMix>slice=saved.stream().filter(x->x.getMixSize()==selectedSize).toList();assertThat(slice).hasSize(3);assertThat(slice).extracting(BestMethodMix::getRank).containsExactly(1,2,3);}
    }
